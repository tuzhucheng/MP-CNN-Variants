...
def run(group, dataset, count, epochs, log_interval, gpu):
    ...
    for _ in range(count):
        ...
        command = "python /u/z3tu/castorini/MP-CNN-Variants/main.py /u/z3tu/castorini/MP-CNN-Variants/saved_models/{model} --arch mpcnn_no_per_dim_no_multi_pooling --optimizer {optimizer} --momentum {momentum} --dataset {dataset} --hidden-units {hidden_units} --log-interval {log_interval} --epochs {epo} --device {dev} --holistic-filters {filters} --batch-size 64 --lr {learning_rate} --regularization {reg} --attention basic --dropout {dropout} --patience {patience} --lr-reduce-factor {lr_reduce} --wide-conv --sparse-features".format(optimizer=optimizer, epo=epochs, model=model_name, dataset=dataset, hidden_units=hidden_units, dev=gpu, log_interval=log_interval, learning_rate=learning_rate, filters=filters, reg=reg, dropout=dropout, patience=patience, lr_reduce=lr_reduce, momentum=momentum)
