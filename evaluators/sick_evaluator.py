...
class SICKEvaluator(Evaluator):
    ...
    def get_scores(self):
        ...
        for batch in self.data_loader:
            ...
            predict_classes = torch.arange(1, num_classes + 1, device=batch.label.device).expand(self.batch_size, num_classes)
            # handle last batch which might have smaller size
            if len(predict_classes) != len(batch.sentence_1):
                predict_classes = torch.arange(1, num_classes + 1, device=batch.label.device).expand(len(batch.sentence_1), num_classes)
    ...
    def get_final_prediction_and_label(self, batch_predictions, batch_labels):
        num_classes = self.dataset_cls.NUM_CLASSES
        predict_classes = torch.arange(1, num_classes + 1, device=batch_labels.device).expand(batch_predictions.size(0), num_classes)
